...
> **顺序很重要**：浏览器密码验证通过后 Gateway 才会生成配对请求。密码/Token 不对时浏览器会直接报 401，服务端 `openclaw devices list` 的 Pending 区一直为空，即使执行 `approve` 也无内容可审批。
...
4. SSH 进 VM（Windows 则是 `wsl -d Ubuntu -u openclaw -- ...`）先列出挂起请求、拿到 Request ID：

```bash
openclaw devices list
# 从 Pending 区找到你刚刚那次请求的 Request 列（UUID），复制下来
```

5. 用该 ID 审批（**注意**：`openclaw devices approve --latest` 在 2026.4.15 版本里只做"预览选中"，并不真正审批，必须显式带 ID）：

```bash
openclaw devices approve <Request-UUID>
```

6. 浏览器自动完成连接；再次 `openclaw devices list` 可看到该设备从 Pending 移入 Paired

### 12.2 常用命令

```bash
openclaw devices list                    # 列出全部设备（含 Pending 与 Paired）
openclaw devices approve <id>            # 审批指定 Request ID（推荐做法）
openclaw devices remove <id>             # 撤销某台已配对设备
openclaw devices remove --all            # 一键清空（所有客户端下次都需重新配对）
```

> **关于 `--latest`**：`openclaw devices approve --latest` 在 2026.4.15 版本只会打印选中的请求信息，并**不会真正审批**，输出末尾会提示 `Approve this exact request with: openclaw devices approve <id>`。请务必用完整 ID 形式。
...
#### 浏览器始终显示 "pairing required"（执行过 `approve` 也没用）

最常见的**真凶**：用了 `openclaw devices approve --latest`，但该命令在 **2026.4.15** 版本里**只做预览、不做审批**。执行后你会看到类似：
...
这行 `Approve this exact request with:` 就是提示你**还没真的审批**，必须把它给出的完整命令再执行一遍。

**正确两步法**：

```bash
# 1. 列出挂起请求，记录 Request 列的 UUID
openclaw devices list

# 2. 用 UUID 真正审批
openclaw devices approve <Request-UUID>

# 3. 再次 list，确认从 Pending 移入 Paired
openclaw devices list
```

如果按上面做完浏览器仍然 "pairing required"，再按下面顺序排查：

**① Pending 是空的 / 看不到你这次登录的请求**

- 密码错了：Gateway 401，Control UI 前端可能把 401 也显示成 "pairing required"。核对 `.env` 里的 `GATEWAY_PASSWORD`（不是 `ADMIN_PASSWORD`）
- Origin 被拦：CORS/同源策略阻止了请求。参见 §11.1 追加 origin
- 浏览器页面没刷新：硬刷新（Cmd+Shift+R / Ctrl+F5）或用无痕窗口重新登录

**② 有多条 Pending**

你之前登录密码错过几次，或换浏览器/清过 Cookie 积累了多个挂起请求。用 `Age` 列找出最新一条（`just now`），把它的 Request UUID 传给 `approve`。

**③ approve 后浏览器没反应**

approve 成功后 Gateway 会通过 WebSocket 推送事件，但偶尔前端没收到：

- 硬刷新页面（Cmd+Shift+R / Ctrl+F5）
- 或清掉该站点 localStorage 后重新登录

**④ 分不清 ADMIN_PASSWORD 和 GATEWAY_PASSWORD**

- `ADMIN_PASSWORD`：**SSH / RDP 登录 VM** 用的
- `GATEWAY_PASSWORD`：**浏览器登录 OpenClaw Web UI** 用的

两个在 `.env` 里是两行，别搞混。

#### 其它常见情况

- **`openclaw devices list` 的 Pending 区一直为空**：请求未能到达 Gateway。检查是否 origin 被拦（参见 §11.1）或密码错误。
